...
* ②调用 `HashMap` 的 hash() 方法进行二次哈希。
  * 二次 hash() 是为了综合高位数据，让哈希分布更为均匀。第一次hash(): 通过对键 🔑 的hashCode()方法的返回值进行运算得到的。  由于可能存在哈希冲突，即不同的键值对可能产生相同的哈希码，因此需要使用二次哈希（又称为再哈希）来解决哈希冲突。 
...
**什么是单例模式？**
 ...
 简单来说，单例模式就是保证一个类只能创建一个对象，且该对象可以在系统的任何地方被访问。 

**单例模式的三个特性：**

- 该类必须有一个私有构造函数，以确保该类不能被其他类直接实例化。
- 该类必须有一个静态方法来获取该类的唯一实例，该方法必须是静态的，以确保可以在类被实例化之前访问该方法。
- 该类必须有一个静态成员变量来存储该类的唯一实例，以确保该实例可以被任何地方访问。
